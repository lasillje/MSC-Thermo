...
do
    ...
    FILE="${HOME}/MPS_FOLDER/${CONDITION}/${FNAME}.mps"
    echo "$FILE"
    # Change the settings for the job submission:
    cat ~/hpc/VA_batch_job_template.sh | sed "s/JOBNAME/$NAME/g" | sed "s/CPUS/$CPU/g"  | sed "s/HOURS/$HOURS/g" | sed "s/MEMORY/$MEMORY/g" > batch_script_${FNAME}.slurm

    # Append line to call Gurobi:
    echo "gurobi.sh ~/hpc/gurobi_optimization_VA.py $FILE $FNAME $HOURS $CPU $OUTPUT_FOLDER" >> batch_script_${FNAME}.slurm
